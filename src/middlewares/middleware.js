...
const autho = async function (req, res, next) {
    try {
        ...
        if (!(req.params.bookId)) {
            ...
            if (!(await userModel.findById(req.body.userId))) return res.status(404).send({ status: false, message: "This Id Doesnot Exists" })
            if (!(req.body.userId == decodedId)) return res.status(403).send({ status: false, message: "You Are Not Authorise to do this" })
        }
        else {
