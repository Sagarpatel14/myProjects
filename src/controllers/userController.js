...
const loginUser = async function (req, res) {
    try {
        ...
        if (!("userName" in body)) return res.status(400).send({ status: false, message: "please enter username" });
        if (!("password" in body)) return res.status(400).send({ status: false, message: "please enter password" });
        if (!isValid(userName)) return res.status(400).send({ status: false, message: "UserName should not be empty" })
        if (!isValid(password)) return res.status(400).send({ status: false, message: "Password should not be empty" })
        ...
        let token = jwt.sign(
            { userId: user._id.toString(), password: user.password },
